...
export default function ContactEdit({ contact, companies, tags, leadTypes, contactStatuses, customFields }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        custom_fields: (contact.custom_fields || {}) as Record<string, string>,
        tags: contact.tags.map((t) => t.id),
    });

    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        put(route('crm.contacts.update', contact.uuid));
    }
        ...
        <CrmLayout>
            ...
            <div className="bg-white border-b border-[#E4E7EB]">
                <div className="flex items-center h-12 px-5">
                    <Link href={route('crm.contacts.show', contact.uuid)} className="text-[#8B9096] hover:text-[#111315] transition-colors mr-3">
                        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
                        </svg>
                    </Link>
                    <h1 className="text-sm font-semibold text-[#111315]">Edit Contact</h1>
                </div>
            </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="text" value={data.first_name} onChange={(e) => setData('first_name', e.target.value)} className={inputClass} required autoFocus />
                    ...
                    <div className="flex items-center justify-end gap-3 pt-3 border-t border-[#E4E7EB]">
                        <Link href={route('crm.contacts.show', contact.uuid)} className="h-9 px-4 flex items-center text-xs font-medium text-[#5F656D] hover:text-[#111315] rounded-[4px] transition-colors">
                            Cancel
                        </Link>
                        <button
                            type="submit"
