...
export default function CompanyEdit({ company }: Props) {
    ...
    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        put(route('crm.companies.update', company.id));
    }
        ...
        <CrmLayout>
            ...
            <div className="bg-white border-b border-[#E4E7EB]">
                <div className="flex items-center h-12 px-5">
                    <Link href={route('crm.companies.show', company.id)} className="text-[#8B9096] hover:text-[#111315] transition-colors mr-3">
                        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
                        </svg>
                    </Link>
                    <h1 className="text-sm font-semibold text-[#111315]">Edit Company</h1>
                </div>
            </div>
            ...
            <div className="max-w-xl mx-auto py-8 px-4">
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="flex items-center justify-end gap-3 pt-3 border-t border-[#E4E7EB]">
                        <Link href={route('crm.companies.show', company.id)} className="h-9 px-4 flex items-center text-xs font-medium text-[#5F656D] hover:text-[#111315] rounded-lg transition-colors">
                            Cancel
                        </Link>
