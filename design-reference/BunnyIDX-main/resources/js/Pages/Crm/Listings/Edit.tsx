...
export default function ListingEdit({ listing, contacts, deals, tags: initialTags, listingTypes: initialListingTypes, listingStatuses: initialListingStatuses, customFields }: Props) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        contact_id: listing.contact_id?.toString() || '',
        deal_id: listing.deal_id?.toString() || '',
        listed_at: listing.listed_at || '',
        sold_at: listing.sold_at || '',
        expired_at: listing.expired_at || '',
        custom_fields: listing.custom_fields || {} as Record<string, string>,
        tags: listing.tags.map((t) => t.id),
        features: {
            property_subtype: existingFeatures.property_subtype || '',
            listing_category: existingFeatures.listing_category || 'for_sale',
            parking_spaces: existingFeatures.parking_spaces || '',
            stories: existingFeatures.stories || '',
            garage_spaces: existingFeatures.garage_spaces || '',
            hoa_fee: existingFeatures.hoa_fee || '',
            amenities: existingFeatures.amenities || ([] as string[]),
            ...existingFeatures,
    ...
    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        put(route('crm.listings.update', listing.id));
    }
            ...
            <div className="flex flex-col h-[calc(100vh-56px)] overflow-hidden bg-[#F3F4F6]">
                ...
                <div className="shrink-0 flex items-center justify-between h-11 bg-white border-b border-[#E4E7EB] px-3 sm:px-6">
                    <div className="flex items-center">
                        <Link href={route('crm.listings.show', listing.id)} className="text-[#8B9096] hover:text-[#111315] transition-colors mr-3">
                            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
                            </svg>
                        </Link>
                        <h1 className="text-xs font-semibold text-[#111315] tracking-wider">Edit Property</h1>
                    </div>
                    <div className="flex items-center gap-3">
                        <Link href={route('crm.listings.show', listing.id)} className="text-[13px] text-[#5F656D] hover:text-[#111315] transition-colors">
                            Cancel
                        </Link>
                        <button
                            type="submit"
                            ...
                            <div className="flex-1 min-w-0 space-y-4">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="relative">
                                                ...
                                                <input type="number" step="0.01" value={data.price} onChange={(e) => setData('price', e.target.value)} className={inputClass + ' pl-7'} />
                                            </div>
                                ...
                                <div className="border border-[#E4E7EB] bg-white rounded-xl p-5">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="w-1/3">
                                            ...
                                            <input type="text" value={data.country} onChange={(e) => setData('country', e.target.value)} maxLength={2} className={inputClass} />
                                        </div>
                                    ...
                                    <div className="space-y-4">
                                        <div className="grid grid-cols-3 gap-4">
                                            <div>
                                                ...
                                                <input type="number" value={data.bedrooms} onChange={(e) => setData('bedrooms', e.target.value)} min="0" className={inputClass} />
                                            </div>
                                            <div>
                                                <label className={labelClass}>Baths</label>
                                                <input type="number" step="0.5" value={data.bathrooms} onChange={(e) => setData('bathrooms', e.target.value)} min="0" className={inputClass} />
                                            </div>
                                            <div>
                                                <label className={labelClass}>Sqft</label>
                                                <input type="number" value={data.sqft} onChange={(e) => setData('sqft', e.target.value)} min="0" className={inputClass} />
                                            </div>
                                        </div>
                                        <div className="grid grid-cols-3 gap-4">
                                            <div>
                                                <label className={labelClass}>Lot Size (acres)</label>
                                                <input type="number" step="0.01" value={data.lot_size} onChange={(e) => setData('lot_size', e.target.value)} min="0" className={inputClass} />
                                            </div>
                                            <div>
                                                <label className={labelClass}>Year Built</label>
                                                <input type="number" value={data.year_built} onChange={(e) => setData('year_built', e.target.value)} min="1800" max="2100" className={inputClass} />
                                            </div>
                                            <div>
                                                <label className={labelClass}>Stories</label>
                                                <input type="number" value={data.features.stories || ''} onChange={(e) => setFeature('stories', e.target.value)} min="0" className={inputClass} />
                                            </div>
                                        </div>
                                        <div className="grid grid-cols-3 gap-4">
                                            <div>
                                                <label className={labelClass}>Parking Spaces</label>
                                                <input type="number" value={data.features.parking_spaces || ''} onChange={(e) => setFeature('parking_spaces', e.target.value)} min="0" className={inputClass} />
                                            </div>
                                            <div>
                                                <label className={labelClass}>Garage Spaces</label>
                                                <input type="number" value={data.features.garage_spaces || ''} onChange={(e) => setFeature('garage_spaces', e.target.value)} min="0" className={inputClass} />
                                            </div>
                                            <div>
                                                <label className={labelClass}>HOA Fee</label>
                                                <div className="relative">
                                                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[13px] text-[#8B9096]">$</span>
                                                    <input type="number" step="0.01" value={data.features.hoa_fee || ''} onChange={(e) => setFeature('hoa_fee', e.target.value)} min="0" className={inputClass + ' pl-7'} />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="border border-[#E4E7EB] bg-white rounded-xl p-5">
                                    ...
                                    <textarea
                                        ...
                                        rows={6}
                                        className="px-3 py-2 text-[13px] border border-[#ECEEF1] bg-white text-[#111315] rounded-lg focus:outline-none focus:border-[#111315] focus:ring-0 w-full resize-none"
                                    />
                                ...
                                <div className="border border-[#E4E7EB] bg-white rounded-xl p-5">
                                    ...
                                    {existingPhotos.length > 0 ? (
                                        ...
                                        <div className="h-[120px] rounded-lg bg-[#F3F4F6] border border-[#E4E7EB] flex flex-col items-center justify-center">
                                            <svg className="h-8 w-8 text-[#D1D5DB] mb-2" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor">
                                            ...
                                            <p className="text-xs text-[#8B9096]">No photos uploaded yet</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                                ...
                                <div className="border border-[#E4E7EB] bg-white rounded-xl p-5">
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <input type="date" value={data.listed_at} onChange={(e) => setData('listed_at', e.target.value)} className={inputClass} />
                                        </div>
                                        <div>
                                            <label className={labelClass}>Sold Date</label>
                                            <input type="date" value={data.sold_at} onChange={(e) => setData('sold_at', e.target.value)} className={inputClass} />
                                        </div>
                                        <div>
                                            <label className={labelClass}>Expired Date</label>
                                            <input type="date" value={data.expired_at} onChange={(e) => setData('expired_at', e.target.value)} className={inputClass} />
                                        </div>
                                    </div>
                                </div>
