...
{{--
    New Developments — the platform-curated pre-construction catalog, grouped
    by area exactly like the Condo Directory (shared cd-* styles + the same
    client-side name search). $groups: area => Collection<NewDevelopment>.
--}}
...
@include('agent-website.partials.blocks.page-header', ['block' => [
    ...
    'data' => [
        ...
        'subtitle' => 'Pre-construction and new projects by area',
        'show_scroll' => false,
    ],
]])
...
<section class="lbp-block cd-page">
    <div class="lbp-inner">
        ...
        <div class="ap-notice">
            <p>New Developments is switched off — visitors get a 404 on this page. Enable it in your website settings. Visitors won&rsquo;t see this message.</p>
        </div>
        @endif

        @if($totalDevelopments === 0)
            <p class="ap-empty">New developments are coming soon. Please check back shortly.</p>
        @else
            @php
                $priceOptions = [
                    '' => 'Any Price',
            ...
            <div class="cd-filters">
                <input id="cd-q" type="text" placeholder="Search developments by name&hellip;" aria-label="Search developments by name">
            ...
            <p class="cd-count" id="cd-count">{{ number_format($totalDevelopments) }} {{ Str::plural('project', $totalDevelopments) }} across {{ $groups->count() }} {{ Str::plural('area', $groups->count()) }}</p>

            <div id="cd-results">
                @foreach($groups as $areaName => $developments)
                <div class="cd-area" data-cd-group>
                    <div class="cd-area-head">
                        <h2 class="cd-area-title">{{ $areaName }}</h2>
                        <span class="cd-area-count">{{ number_format($developments->count()) }} {{ Str::plural('project', $developments->count()) }}</span>
                    </div>
                    <div class="cd-grid">
                        @foreach($developments as $development)
                        <a class="cd-card" href="{{ route('agent-site.new-developments.show', [$site->slug, $development->slug]) }}" data-cd-name="{{ Str::lower($development->name) }}" data-cd-area="{{ $areaName }}" data-cd-price="{{ $development->priceFromValue() ?? '' }}">
                            <div class="cd-card-media">
                                @if($development->image)
                                    <img src="{{ Str::startsWith($development->image, ['http://', 'https://']) ? $development->image : asset('storage/' . $development->image) }}" alt="{{ $development->name }}" loading="lazy">
                                @else
                                    <span class="cd-card-placeholder" aria-hidden="true"></span>
                                @endif
                            </div>
                            <div class="cd-card-body">
                                <h3 class="cd-card-name notranslate" translate="no">{{ $development->name }}</h3>
                                <p class="cd-card-addr">
                                    {{ $development->statusLabel() }}@if($development->completion_year) &middot; Est. {{ $development->completion_year }}@endif
                                    @if($development->price_label) &middot; {{ $development->price_label }}@endif
                                </p>
                            </div>
                        </a>
                        @endforeach
                ...
                <p class="cd-no-results" id="cd-no-results" hidden>No developments match those filters.</p>
            </div>

            <div id="cd-map" class="cd-map" hidden aria-label="Map of new developments"></div>

            @php
                // Marker data for the map view — only projects with coordinates.
                $mapProjects = $groups->flatten()
                    ...
                    ->map(fn ($d) => [
                        ...
                        'image' => $d->image
                            ...
                            : null,
                        'url' => route('agent-site.new-developments.show', [$site->slug, $d->slug]),
                    ])->values();
            @endphp
            <script type="application/json" id="cd-map-data">@json($mapProjects)</script>
...
@include('agent-website.partials.blocks.cta', ['block' => ['data' => [
    ...
    'heading' => 'Interested in Pre-Construction?',
    'description' => 'Get project-level guidance, floor plans and priority access to developer inventory.',
...
@php
    $breadcrumbLd = [
        ...
        'itemListElement' => [
            ...
            ['@type' => 'ListItem', 'position' => 2, 'name' => 'New Developments', 'item' => route('agent-site.new-developments', $site->slug)],
        ],
    ];
@endphp
