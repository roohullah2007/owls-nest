...
{{--
    One Condo Directory building — duplicate of the New Development project
    page (hero logo, facts, actions, highlights, gallery, floor plans,
    developer, map, lead modal). $building: CondoBuilding.
--}}
...
@php
    ...
    $imgUrl = $mediaUrl($building->image);
    $logoUrl = $mediaUrl($building->logo);
    $brochureUrl = $mediaUrl($building->brochure);

    $searchUrl = route('agent-site.properties', $site->slug) . '?q=' . urlencode($building->listingKeyword());

    $listingsTotal = (int) ($listingsTotal ?? 0);
    $highlights = array_values(array_filter((array) ($building->highlights ?? [])));

    // Owner/admin-entered structured extras.
    $keyDetails = array_values(array_filter((array) ($building->key_details ?? []), fn ($d) => ! empty($d['label']) && ! empty($d['value'])));
    $gallery = array_values(array_filter((array) ($building->gallery ?? [])));
    $floorPlans = array_values(array_filter((array) ($building->floor_plans ?? []), fn ($f) => ! empty($f['image'])));
    $depositSchedule = array_values(array_filter(array_map('trim', (array) ($building->deposit_schedule ?? []))));
    ...
    $facts = array_filter([
        ...
        'Interior Design' => $building->interior_design,
        'Est. Completion' => $building->completion_year,
        'Pricing' => $building->price_label,
    ...
    $developerProfile = $building->developerProfile;
    $developerLogoUrl = $mediaUrl($developerProfile?->logo);
    $developerInfo = trim((string) ($developerProfile?->info ?: $building->developer_info));

    $crumbItems = [
        ['label' => 'Home', 'url' => route('agent-site.home', $site->slug)],
        ['label' => 'Condo Directory', 'url' => route('agent-site.condos', $site->slug)],
        ['label' => $building->name],
    ];
@endphp
...
@include('agent-website.partials.blocks.page-header', ['block' => [
    'id' => 'condo-building-hero-' . $building->slug,
    'crumbs' => $crumbItems,
    'data' => [
        'bg_type' => 'image',
        'image' => $imgUrl,
        ...
        'subtitle' => trim(implode(' · ', array_filter([$building->statusLabel(), $building->area, $building->price_label]))),
        'show_scroll' => false,
    ],
]])
...
<section class="lbp-block ap-page">
    <div class="lbp-inner">
        ...
        <div class="nd-actions">
            ...
            <a class="nd-action-btn" href="{{ $building->video_url }}" target="_blank" rel="noopener">Watch Video</a>
            @endif
        </div>

        @php $hasSide = ! empty($keyDetails) || ! empty($depositSchedule); @endphp
        @if(trim((string) $building->description) !== '' || $hasSide)
        <div class="ap-guide{{ $hasSide ? '' : ' ap-guide--solo' }}">
            @if(trim((string) $building->description) !== '')
            <div class="ap-rich">
                <h2>About {{ $building->name }}</h2>
                @if(Str::contains($building->description, '<'))
                    {!! strip_tags($building->description, '<p><br><strong><b><em><i><u><ul><ol><li><a><h3><h4><blockquote>') !!}
                @else
                    {!! nl2br(e($building->description)) !!}
                @endif
                @if($building->video_url)
                <p><a href="{{ $building->video_url }}" target="_blank" rel="noopener">Watch the building video</a></p>
                @endif
            </div>
            @endif
        ...
        <div class="ap-section">
            ...
            <div class="nd-gallery">
                ...
                <button type="button" class="nd-gallery-item" data-nd-lightbox="gallery" data-nd-index="{{ $i }}" aria-label="Open photo {{ $i + 1 }}">
                    <img src="{{ $mediaUrl($photo) }}" alt="{{ $building->name }} — photo {{ $i + 1 }}" loading="lazy">
                </button>
                @endforeach
            </div>
        </div>
        ...
        <div class="ap-section" id="floor-plans">
            ...
            <div class="nd-floorplans">
                ...
                <button type="button" class="nd-floorplan" data-nd-lightbox="plans" data-nd-index="{{ $i }}" aria-label="Open floor plan {{ $plan['label'] ?? $i + 1 }}">
                    <span class="nd-floorplan-media"><img src="{{ $mediaUrl($plan['image']) }}" alt="{{ $plan['label'] ?? $building->name . ' floor plan' }}" loading="lazy"></span>
        ...
        {{-- About the Developer --}}
        @if($building->developer && $developerInfo !== '')
        <div class="ap-section">
            <h2 class="ap-section-title">About {{ $building->developer }}</h2>
            <div class="nd-developer{{ $developerLogoUrl ? ' nd-developer--logo' : '' }}">
                @if($developerLogoUrl)
                <div class="nd-developer-logo"><img src="{{ $developerLogoUrl }}" alt="{{ $building->developer }} logo"></div>
                @endif
        ...
        {{-- More buildings by the same developer --}}
        @if($moreByDeveloper->isNotEmpty())
        <div class="ap-section">
            <h2 class="ap-section-title">More by {{ $building->developer }}</h2>
            <div class="cd-grid">
                @foreach($moreByDeveloper as $project)
                <a class="cd-card" href="{{ route('agent-site.condos.building', [$site->slug, $project->slug]) }}">
        ...
        @if(! empty($buildingListings))
        <div class="ap-section ap-listings">
            <div class="ap-listings-head">
                <h2 class="ap-section-title">Listings at {{ $building->name }}</h2>
                @if($listingsTotal > 0)
            ...
            <div class="ap-grid">
                @foreach($buildingListings as $prop)
                    @include('agent-website.templates.luxury.area-card', ['prop' => $prop])
                @endforeach
            </div>

            <div class="ap-search-cta">
                <a class="ap-search-btn" href="{{ $searchUrl }}">Search Homes near {{ $building->name }}</a>
            </div>
        </div>
        @endif
    </div>
...
<section class="nd-map-section">
    <div class="lbp-inner nd-map-head">
        ...
        @if($building->fullAddress())<p class="nd-map-addr">{{ $building->fullAddress() }}</p>@endif
    </div>
    <div id="nd-map" class="nd-map"
         data-lat="{{ $building->lat }}"
         data-lng="{{ $building->lng }}"
         data-pin="{{ $building->name }}"
         aria-label="Map showing {{ $building->name }}"></div>
</section>
<script>
(function () {
...
<section class="nd-disclaimer-section">
    <div class="lbp-inner">
        <p class="nd-disclaimer">
            ...
            videos and similar materials — is provided by or sourced from the building&rsquo;s developer to
            facilitate the marketing of the building; {{ $siteName }} does not assert ownership or creation of
...
<div class="nd-req-modal" id="nd-req-modal" hidden role="dialog" aria-modal="true" aria-labelledby="nd-req-title">
    <div class="nd-req-card">
        ...
        <p class="nd-req-sub">{{ $building->name }} &middot; {{ $building->area }}</p>
        <form method="POST" action="{{ route('agent-site.contact.submit', $site->slug) }}">
            @csrf
            <input type="hidden" name="interest" id="nd-req-interest" value="Information request — {{ $building->name }}">
            ...
            <div class="nd-req-field">
                ...
                <textarea id="nd-req-message" name="message" rows="4">I'd like more information about {{ $building->name }}.</textarea>
            </div>
...
(function () {
    ...
    var devName = @json($building->name);
    function open(kind) {
...
@include('agent-website.partials.blocks.cta', ['block' => ['data' => [
    ...
    'heading' => "Interested in {$building->name}?",
...
@php
    $lightboxSets = [
        'gallery' => array_map(fn ($p, $i) => ['src' => $mediaUrl($p), 'caption' => $building->name . ' — photo ' . ($i + 1)], $gallery, array_keys($gallery)),
...
@php
    $selfUrl = route('agent-site.condos.building', [$site->slug, $building->slug]);
    $breadcrumbLd = [
    ...
    $placeLd = array_filter([
        ...
        'description' => $building->description ? Str::limit(trim(preg_replace('/\s+/', ' ', strip_tags($building->description))), 300) : null,
    ]);
@endphp
