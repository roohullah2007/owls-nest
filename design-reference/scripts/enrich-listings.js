...
if (!m) {
 console.error('LISTINGS array not found'); process.exit(1); 
}
...
rec.status = st;
 ...
 fail++; rec.photos = [p.img]; 
  ...
  if ((k + 1) % 10 === 0) {
console.log((k + 1) + '/' + LISTINGS.length + '  ok=' + ok + ' fail=' + fail);
}
}
